...
/*
 * Setup lookup methods.
 * Currently exact-match
 * is only supported.
 */
static void
setup_lbtestbed_lookup_tables(void)
{
	/* Setup HASH lookup functions. */
	if (lbtestbed_em_on)
		lbtestbed_lkp = lbtestbed_em_lkp;
}
...
prepare_ptype_parser(uint16_t portid, uint16_t queueid)
{
	...
	if (lbtestbed_lkp.check_ptype(portid))
		return 1;
...
main(int argc, char **argv)
{
	...
	if (ret < 0)
		rte_exit(EXIT_FAILURE, "Invalid LBTESTBED parameters\n");
	...
	rte_eal_mp_remote_launch(lbtestbed_lkp.main_loop, NULL, CALL_MASTER);
